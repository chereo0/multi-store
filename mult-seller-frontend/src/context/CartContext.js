...
import { addToCart as apiAddToCart, emptyCart as apiEmptyCart, removeFromCartAPI, updateCartAPI } from '../api/services';
...
export const CartProvider = ({ children }) => {
  const [cartItems, setCartItems] = useState(() => {
    ...
    console.log('CartProvider: Initial state - savedCart from localStorage:', savedCart);
    if (savedCart) {
      try {
  ...
  const addToCart = async (product, storeId, quantity = 1) => {
    ...
    try {
      ...
      if (!isSuccess) {
        ...
        // Handle error - can be a string, array, or object
      ...
      if (res.data && Array.isArray(res.data.items)) {
        const normalized = res.data.items.map((it) => {
          const productObj = it.product || it.product_detail || {
            id: it.product_id || it.productId,
            name: it.name || it.title,
            price: it.price || it.unit_price,
            image: it.image || (it.product && it.product.image) || '/no-image.png',
          };
      ...
      setCartItems(backupRef.current || []);
      const message = err?.message || String(err) || 'Network error adding to cart';
      toast.error(message);
  ...
  const updateQuantity = async (productId, storeId, quantity) => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        // Revert if server failed
        setCartItems(backup);
        const errorMsg = res?.error || res?.message || 'Failed to update quantity';
        if (Array.isArray(res?.error)) {
          toast.error(res.error.filter(Boolean).join(' '));
        } else {
          toast.error(errorMsg);
